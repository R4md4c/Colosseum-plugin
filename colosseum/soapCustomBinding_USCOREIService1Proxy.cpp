...
int CustomBinding_USCOREIService1Proxy::LinesCount(_ns1__LinesCount *ns1__LinesCount, _ns1__LinesCountResponse *ns1__LinesCountResponse)
{	struct soap *soap = this;
	struct __ns1__LinesCount soap_tmp___ns1__LinesCount;
	const char *soap_action = NULL;
	if (!soap_endpoint)
		soap_endpoint = "http://localhost:2222/Service1.svc";
	soap_action = "http://tempuri.org/IService1/LinesCount";
	soap->encodingStyle = NULL;
	soap_tmp___ns1__LinesCount.ns1__LinesCount = ns1__LinesCount;
	soap_begin(soap);
	soap_serializeheader(soap);
	soap_serialize___ns1__LinesCount(soap, &soap_tmp___ns1__LinesCount);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put___ns1__LinesCount(soap, &soap_tmp___ns1__LinesCount, "-ns1:LinesCount", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	if (soap_connect(soap, soap_endpoint, soap_action)
	 ...
	 || soap_end_send(soap))
		return soap_closesock(soap);
	if (!ns1__LinesCountResponse)
		return soap_closesock(soap);
	ns1__LinesCountResponse->soap_default(soap);
	if (soap_begin_recv(soap)
	 || soap_envelope_begin_in(soap)
	 || soap_recv_header(soap)
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	ns1__LinesCountResponse->soap_get(soap, "ns1:LinesCountResponse", "");
	if (soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
