...
HRESULT CDXUTDialogResourceManager::createStateBlock()
{
	HRESULT hr = S_OK;
	V_RETURN( m_pd3d9Device->CreateStateBlock( D3DSBT_ALL, &m_pStateBlock) );
	return S_OK;
    ...
    switch( uMsg )
    {
        ...
        case WM_LBUTTONDBLCLK:
            {
                if( ContainsPoint( pt ) )
                {
					...
					/* Send Button Clicked while the button is down */
					m_pDialog->SendEvent( EVENT_BUTTON_DOWN, true, this );
                    return true;
                }
        ...
        case WM_LBUTTONUP:
        {
            if( m_bPressed )
            {
                ...
                // Button click
                if( ContainsPoint( pt ) )
                    m_pDialog->SendEvent( EVENT_BUTTON_UP, true, this );
